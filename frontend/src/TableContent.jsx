...
const TableContent = ({ tableData, onCellClick, children }) => {
  const getBorderStyles = (colIndex, rowIndex, numCols, numRows, minHeight) => {
    ...
    rowIndex <= -1 &&
      colIndex >= numCols - 1 &&
      (stypes['borderTopRightRadius'] = '10px');
    ...
    rowIndex >= numRows - 1 &&
      numRows > 0 &&
      colIndex <= 0 &&
      (stypes['borderBottomLeftRadius'] = '11px');
    rowIndex >= numRows - 1 &&
      numRows > 0 &&
      colIndex >= numCols - 1 &&
      (stypes['borderBottomRightRadius'] = '11px');
    ...
    if (numRows <= 0 && minHeight <= 0) {
      ...
      rowIndex <= -1 &&
        colIndex <= 0 &&
        (stypes['borderBottomLeftRadius'] = '10px');
      rowIndex <= -1 &&
        colIndex >= numCols - 1 &&
        (stypes['borderBottomRightRadius'] = '10px');
    }
    ...
    <>
      ...
      >
        ...
        <div className="table-content-data">
          {tableData.rowsData.map(
            (rowData, rowIndex) =>
              rowIndex < tableData.numRows && (
                <div className="table-row-data" key={rowIndex}>
                  {rowData.map((cellData, colIndex) => (
                    <React.Fragment key={colIndex}>
                      {colIndex < tableData.numCols &&
                        tableData.dataTypes[colIndex] ===
