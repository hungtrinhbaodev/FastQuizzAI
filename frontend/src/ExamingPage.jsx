...
const ExamingPage = ({iExamingData, useDialog, onFinishExam, closeDialog}) => {
    ...
    const onSubmitExaming = () => {
        ...
        if (answersIndex.filter((answersIndex, index) => (answersIndex < 0 || answersIndex > questionsData[index].options.length)).length) {
            const dialogData = useDialog(DialogData.makeConfirmNotify("You don't chose all questions, do you want to submit!", () => {
    ...
    return (
        ...
        >
            <div
                className='examing-page-time-countdown-container'
            >
                {AppUtils.convertMiliToTimeString(timeRemain)}
            </div>
            <h1
                className='examing-page-title'
            >
                {examName !== "" && ("Examing in exam: " + examName)}
            </h1>
            {questionsData.length > 0 && questionsData.map((questionData, index) => {
                return (
                <BoxExamQuestion
                    key={index}
            ...
            <div
                className='examing-page-button-group-container'
            >
                <button
